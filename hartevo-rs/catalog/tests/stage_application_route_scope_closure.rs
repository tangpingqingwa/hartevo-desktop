...
fn assert_contract_binding(catalog: &Catalog, scopes: &[StageApplicationRouteScope]) {
    ...
    assert_eq!(contract.contract_version, "desktop-2026-09-07-ct04-v10");
...
fn assert_foundation_and_beta_scope(scopes: &[StageApplicationRouteScope]) {
    ...
    assert_eq!(foundation.summary.implemented_handler_count, 25);
    assert_eq!(foundation.summary.not_implemented_handler_count, 4);
    ...
    assert_eq!(beta.summary.implemented_handler_count, 8);
    assert_eq!(beta.summary.not_implemented_handler_count, 14);
...
fn assert_ga_handler_scope(ga: &StageApplicationRouteScope) {
    ...
    assert_eq!(ga.summary.implemented_handler_count, 25);
    assert_eq!(ga.summary.not_implemented_handler_count, 27);
            ...
            (
                ...
                handler.handler_id == *handler_id
                    && matches!(
                        ...
                        "VM-00" | "VM-01" | "VM-03" | "VM-04" | "VM-07" | "VM-11"
                    )
            }
            (
...
fn assert_terminal_and_runtime_boundaries(catalog: &Catalog, ga: &StageApplicationRouteScope) {
    ...
    let vm07 = ga
        .mission_scopes
        .iter()
        .find(|mission| mission.mission_id == "VM-07")
        .expect("VM-07 scope");
    let replan_or_terminal = vm07
        .application_routes
        .iter()
        .find(|route| route.route.checkpoint_id == "replan_or_terminal")
        .expect("VM-07 replan-or-terminal route");
    assert_eq!(
        replan_or_terminal.handler.status,
        StageApplicationHandlerStatus::Implemented
    );
    assert!(matches!(
        replan_or_terminal.terminal_transition_authorities[0].authority,
        RouteTerminalExecutionAuthority::ApplicationHandler(_)
    ));
