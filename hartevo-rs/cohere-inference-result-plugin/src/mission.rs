...
use crate::{
    digest_serializable,
    model::{
        CohereInferenceError, CohereInferenceScope, EvidenceDisposition, InferenceResultEvidence,
        InferenceResultProposal, InferenceResultState, ModelDescription, PluginRegistration,
        Revocation, RevocationReason,
    },
    provider::{CohereProvider, RecordedCohereResponse},
    service::CohereInferenceResultService,
};
...
pub struct MissionCohereInferenceProjection {
...
pub type MissionResultProjection = MissionCohereInferenceProjection;

impl MissionCohereInferenceProjection {
    fn from_result(
...
/// Mission consumer for one exact Cohere model revision/provider route.
#[derive(Clone, Debug)]
pub struct MissionCohereInferenceConsumer {
    service: CohereInferenceResultService,
}

impl MissionCohereInferenceConsumer {
    pub fn new(
        scope: CohereInferenceScope,
        provider: CohereProvider,
    ) -> Result<Self, CohereInferenceError> {
        Ok(Self {
            service: CohereInferenceResultService::new(scope, provider)?,
        })
    }

    pub fn from_service(service: CohereInferenceResultService) -> Self {
        Self { service }
    }

    pub fn service(&self) -> &CohereInferenceResultService {
        &self.service
    }

    pub fn service_mut(&mut self) -> &mut CohereInferenceResultService {
        &mut self.service
    }
    ...
    pub fn describe_model(&self) -> Result<ModelDescription, CohereInferenceError> {
    ...
    pub fn consume_recorded_result(
    ...
    ) -> Result<MissionCohereInferenceProjection, CohereInferenceError> {
        ...
        Ok(MissionCohereInferenceProjection::from_result(
            self.service.scope(),
            proposal,
            &evidence,
        ))
