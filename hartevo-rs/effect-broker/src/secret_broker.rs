...
//! secret bytes.  A consumer can pass a live handle to a registered provider
//! adapter, but the resulting receipt carries no Effect authority.
...
impl SecretBrokerConsumer {
    /// Passes only the opaque handle to a registered provider adapter.
    pub fn use_with_provider<P: SecretBrokerProvider>(
        &self,
        ...
        provider
            .use_opaque_credential(handle)
            .map_err(SecretBrokerError::ProviderRejected)?;
...
mod tests {
    ...
    fn handle(service: &SecretBrokerService) -> SecretUseHandle {
        service
            .issue_handle(
                &consumer(),
                &registry(adapter(1)),
    ...
    fn cross_scope_consumer_and_adapter_replay_is_rejected() {
        ...
        let mut wrong_provider = TestProvider {
            identity: ProviderAdapterIdentity::new("hartevo.sorftime", 1).expect("provider"),
            last_handle_digest: None,
            last_reference_id: None,
        };
        assert_eq!(
    ...
    fn unmount_revoke_and_crash_invalidate_immediately() {
        ...
        let old_handle = handle(&unmounted);
    ...
    fn debug_and_events_redact_reference_and_handle_content() {
        let mut service = mounted_service();
        let handle = handle(&service);
        let debug = format!("{:?} {:?} {:?}", service.reference(), service, handle);
