//! Typed, bounded, serializable contract models for the Mistral result seam.
...
use crate::{
    ...
    MISTRAL_INFERENCE_PLUGIN_VERSION, MISTRAL_INFERENCE_PROVIDER_ID,
    MISTRAL_INFERENCE_SCHEMA_VERSION, MISTRAL_INFERENCE_SERVICE_ID, digest_bytes,
    digest_serializable,
};

pub const DEFAULT_MISTRAL_API_HOST: &str = "https://api.mistral.ai";
...
pub const MAX_MODEL_LIST_ITEMS: usize = 256;
pub const MAX_EMBEDDING_DIMENSIONS: usize = 16_384;
pub const MAX_CLASSIFICATION_RESULTS: usize = 64;
...
pub enum MistralInferenceError {
    ...
    StreamingForbidden,
    #[error("file inputs and file authority are not allowed by the Layer-1 contract")]
    ...
    #[error("provider model-list did not contain the pinned model")]
    ModelNotAllowlisted,
...
impl SecretReference {
    pub fn new(
        ...
        let mut material = b"hartevo:mistral-secret-reference:v1:".to_vec();
...
#[serde(transparent)]
pub struct MistralApiHost(String);

impl MistralApiHost {
    pub fn new(value: impl Into<String>) -> Result<Self, MistralInferenceError> {
        let value = value.into().trim_end_matches('/').to_owned();
        if value != DEFAULT_MISTRAL_API_HOST
            || value.contains('?')
            || value.contains('#')
            || value.chars().any(char::is_whitespace)
        {
            return Err(MistralInferenceError::InvalidField {
                field: "api_host",
                reason: "must be the HTTPS Mistral API host without query or fragment",
            });
        }
        Ok(Self(value))
    }
    ...
    pub fn api() -> Self {
        Self(DEFAULT_MISTRAL_API_HOST.to_owned())
    }
...
impl fmt::Debug for MistralApiHost {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_tuple("MistralApiHost")
            .field(&self.0)
            .finish()
    }
}
...
impl ModelRevision {
    pub fn new(
        ...
        if model_id.trim().is_empty()
            ...
            || model_id.chars().any(|character| character.is_whitespace())
            || model_id.chars().any(char::is_control)
        {
            return Err(MistralInferenceError::InvalidField {
                field: "model_id",
                reason: "must be a bounded non-empty Mistral model identifier",
            });
        }
...
pub type MistralModelRevision = ModelRevision;
...
pub type MistralTask = InferenceTask;

#[derive(Clone, Deserialize, Eq, PartialEq, Serialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct ProviderRoute {
    provider_id: String,
    api_host: MistralApiHost,
}

impl ProviderRoute {
    pub fn new(api_host: MistralApiHost) -> Result<Self, MistralInferenceError> {
        if api_host.as_str() != DEFAULT_MISTRAL_API_HOST {
            return Err(MistralInferenceError::InvalidField {
                field: "provider_route",
                reason: "only the explicit Mistral API route is allowlisted",
            });
        }
        Ok(Self {
            provider_id: "mistral".to_owned(),
            api_host,
        })
    }
    ...
    pub fn api_host(&self) -> &MistralApiHost {
        &self.api_host
    }
...
pub type MistralProviderRoute = ProviderRoute;

/// Scope account details intentionally do not implement serialization because
/// they contain the opaque secret binding.
#[derive(Clone, Eq, PartialEq)]
...
impl ProjectScope {
    pub fn new(id: impl Into<String>, revision: u64) -> Result<Self, MistralInferenceError> {
...
fn scoped_id<T>(
    ...
    id: impl Into<String>,
    revision: u64,
) -> Result<T, MistralInferenceError>
where
    T: From<ScopedId>,
{
    let id = id.into();
...
pub struct InferencePolicy {
    ...
    max_classification_results: usize,
}
...
impl InferencePolicy {
    ...
    pub fn new(
        ...
        if max_input_bytes == 0
            ...
            || max_model_list_items == 0
            || max_model_list_items > MAX_MODEL_LIST_ITEMS
            || max_classification_results == 0
            || max_classification_results > MAX_CLASSIFICATION_RESULTS
        {
            return Err(MistralInferenceError::InvalidField {
    ...
    pub fn bounded(revision: impl Into<String>) -> Result<Self, MistralInferenceError> {
        Self::new(
            revision,
            64 * 1024,
            16,
    ...
    pub const fn max_model_list_items(&self) -> usize {
        self.max_model_list_items
    }

    pub const fn max_classification_results(&self) -> usize {
        self.max_classification_results
    }
...
pub type MistralInferencePolicy = InferencePolicy;

#[derive(Clone, Eq, PartialEq)]
pub struct MistralInferenceScope {
    api_host: MistralApiHost,
...
impl MistralInferenceScope {
    ...
    pub fn new(
        ...
        if provider_route.provider_id() != "mistral" {
...
impl fmt::Debug for MistralInferenceScope {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("MistralInferenceScope")
            .field("api_host", &self.api_host)
...
pub type MistralScope = MistralInferenceScope;

#[derive(Serialize)]
struct ScopeDigestMaterial<'a> {
    api_host: &'a MistralApiHost,
...
impl PluginRegistration {
    pub(crate) fn new(scope: &MistralInferenceScope) -> Self {
        let version_digest = digest_serializable(&(
            MISTRAL_INFERENCE_PLUGIN_VERSION,
            MISTRAL_INFERENCE_CONTRACT_VERSION,
        ));
        let contract_digest = digest_bytes(MISTRAL_INFERENCE_CONTRACT_JSON.as_bytes());
        ...
        let registration_digest = digest_serializable(&RegistrationMaterial {
            plugin_version: MISTRAL_INFERENCE_PLUGIN_VERSION,
            contract_version: MISTRAL_INFERENCE_CONTRACT_VERSION,
        ...
        Self {
            plugin_version: MISTRAL_INFERENCE_PLUGIN_VERSION.to_owned(),
            contract_version: MISTRAL_INFERENCE_CONTRACT_VERSION.to_owned(),
            version_digest,
            contract_digest,
            provider_digest,
            model_digest,
    ...
    pub(crate) fn validate_against(
        ...
        let expected_contract = digest_bytes(MISTRAL_INFERENCE_CONTRACT_JSON.as_bytes());
        ...
        {
            return Err(MistralInferenceError::RegistrationTampered);
        }
        if !self.is_active() {
            return Err(MistralInferenceError::RegistrationRevoked);
        }
        Ok(())
    }
    ...
    pub(crate) fn revoke(
        ...
        if self.is_active() {
            self.revocation_revision = self.revocation_revision.checked_add(1).ok_or(
                MistralInferenceError::InvalidField {
    ...
    pub(crate) fn restore(&mut self) -> Result<(), MistralInferenceError> {
        if !self.is_active() {
            self.revocation_revision = self.revocation_revision.checked_add(1).ok_or(
                MistralInferenceError::InvalidField {
                    field: "revocation_revision",
...
impl ChatMessage {
    pub fn new(role: ChatRole, content: impl Into<String>) -> Result<Self, MistralInferenceError> {
...
impl InferenceInput {
    pub fn chat(messages: Vec<ChatMessage>) -> Result<Self, MistralInferenceError> {
    ...
    pub fn texts<I, S>(texts: I) -> Result<Self, MistralInferenceError>
    where
        ...
        S: Into<String>,
    {
        ...
        if texts.iter().any(|text| {
            text.is_empty() || text.len() > MAX_ITEM_BYTES || text.chars().any(char::is_control)
        }) {
    ...
    pub fn text(input: impl Into<String>) -> Result<Self, MistralInferenceError> {
        Self::texts([input.into()])
    }
...
impl GenerationBudget {
    pub fn new(max_new_tokens: u32) -> Result<Self, MistralInferenceError> {
...
impl InferenceRequestFingerprint {
    pub(crate) fn from_request(scope: &MistralInferenceScope, request: &InferenceRequest) -> Self {
...
impl<'a> CanonicalRequest<'a> {
    fn from_request(scope: &'a MistralInferenceScope, request: &'a InferenceRequest) -> Self {
...
pub type MistralInferenceResultProposal = InferenceResultProposal;
...
impl InferenceResultProposal {
    pub(crate) fn new(
        ...
        let mut proposal = Self {
            proposal_version: "mistral-inference-result-proposal/v1".to_owned(),
            service_id: MISTRAL_INFERENCE_SERVICE_ID.to_owned(),
            task: scope.task,
            ...
            proposal_digest: digest_bytes(b"uninitialized-mistral-proposal-digest"),
        };
    ...
    pub fn verify_integrity(&self) -> Result<(), MistralInferenceError> {
        if self.proposal_digest == self.compute_digest() {
            Ok(())
        } else {
            Err(MistralInferenceError::ProposalTampered)
        }
    }
}
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct ClassificationProjection {
...
impl InferenceResultEvidence {
    ...
    pub(crate) fn new(
        ...
        let mut evidence = Self {
            evidence_version: "mistral-inference-result-evidence/v1".to_owned(),
            recording_key,
            ...
            evidence_digest: digest_bytes(b"uninitialized-mistral-evidence-digest"),
        };
    ...
    pub fn compute_digest(&self) -> Digest {
        digest_serializable(&EvidenceMaterial {
            ...
            classification: self.classification.as_ref(),
    ...
    pub fn verify_integrity(&self) -> Result<(), MistralInferenceError> {
        if self.authority.connected() || self.authority.native() || self.authority.first_party() {
            return Err(MistralInferenceError::EvidenceTampered);
        }
        if self.result_revision == 0 || self.evidence_digest != self.compute_digest() {
            return Err(MistralInferenceError::EvidenceTampered);
        }
        Ok(())
    }
...
struct EvidenceMaterial<'a> {
    ...
    classification: Option<&'a ClassificationProjection>,
...
impl ModelDescription {
    pub(crate) fn from_scope(scope: &MistralInferenceScope) -> Self {
        Self {
            model: scope.model.clone(),
            task: scope.task,
            provider_route: scope.provider_route.clone(),
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct ModelListEvidence {
    pub evidence_version: String,
    pub recording_key: Digest,
    pub response_digest: Digest,
    pub result_revision: u64,
    pub models: Vec<ModelListItem>,
    pub model_list_digest: Digest,
    pub pinned_model_allowlisted: bool,
    pub authority: EvidenceAuthority,
    pub evidence_digest: Digest,
}

impl ModelListEvidence {
    pub(crate) fn new(
        mode: ProviderMode,
        recording_key: Digest,
        response_digest: Digest,
        ...
        pinned_model_allowlisted: bool,
    ) -> Self {
        let model_list_digest = digest_serializable(&models);
        let mut evidence = Self {
            evidence_version: "mistral-model-list-evidence/v1".to_owned(),
            recording_key,
            response_digest,
            result_revision,
            models,
            ...
            authority: EvidenceAuthority::for_mode(mode),
            evidence_digest: digest_bytes(b"uninitialized-mistral-model-list-digest"),
        };
        evidence.evidence_digest = evidence.compute_digest();
        evidence
    }
    ...
    pub fn compute_digest(&self) -> Digest {
        digest_serializable(&ModelListMaterial {
            evidence_version: &self.evidence_version,
            recording_key: &self.recording_key,
            response_digest: &self.response_digest,
            result_revision: self.result_revision,
            models: &self.models,
            model_list_digest: &self.model_list_digest,
            pinned_model_allowlisted: self.pinned_model_allowlisted,
            authority: &self.authority,
        })
    }
...
#[derive(Serialize)]
struct ModelListMaterial<'a> {
    evidence_version: &'a str,
    recording_key: &'a Digest,
    response_digest: &'a Digest,
    result_revision: u64,
    models: &'a [ModelListItem],
    model_list_digest: &'a Digest,
    pinned_model_allowlisted: bool,
    authority: &'a EvidenceAuthority,
}

/// Compile-time assertion helpers for callers that need the exact seam ids.
pub const fn provider_identity() -> &'static str {
    MISTRAL_INFERENCE_PROVIDER_ID
}

pub const fn contract_schema_identity() -> &'static str {
    MISTRAL_INFERENCE_SCHEMA_VERSION
}

pub const fn service_identity() -> &'static str {
    MISTRAL_INFERENCE_SERVICE_ID
}
