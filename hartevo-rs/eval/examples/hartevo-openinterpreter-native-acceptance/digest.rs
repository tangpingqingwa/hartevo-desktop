...
mod tests {
    ...
    fn digests_are_domain_separated_lowercase_sha256() {
        let first = digest_json("oi-native-test", &serde_json::json!({"value": 1})).unwrap();
        let second = digest_json("oi-native-other", &serde_json::json!({"value": 1})).unwrap();
        assert_ne!(first, second);
        assert!(is_lower_hex(&first, 32));
        assert_eq!(sha256_hex(b"oi-native"), sha256_hex(b"oi-native"));
