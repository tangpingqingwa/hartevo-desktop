...
v40～v47 还实现了本地 durable Mission Scheduler 与精确 Checkpoint 路由的 E2 切片：连续型 Outcome 与下一周期 Schedule 同一事务；interval、event-driven 与 interval-or-event cadence 被强类型化；Conversation signed inbound 与首次事件 signal 同一事务；claim/heartbeat/failure 只接受 exact owner/token digest、generation 与 lease；Catalog 周期只按 n→n+1 重置已终结 DAG。Catalog v10 的 123 个 Checkpoint 均按 DAG 顺序绑定 Capability、executor、精确 Oracle 子集与 completion policy，Application 创建/推进的 Task 必须匹配当前路由；Task/Checkpoint/Event/Outbox 原子提交，旧的未绑定定义只可审计读取，Scheduler 会 `DeadLetter + Partial` 而不会猜 Capability。本地 Runtime 只接受 executor=`runtime`。`dispatch_current_mission_checkpoint` 会把 DAG 中下一个 `Ready` 节点连同 exact Task 原子启动，并返回 revision-fenced proof；Desktop 只有在 Runtime+Ready 时进入 OpenInterpreter。Human route 现在有两个窄而真实的边界：VM-07 `ConfirmHumanMissionCheckpoint` 把用户陈述、双 revision CAS、可选 WorkProduct、完成和下一 route Task 放在同一事务；VM-11 结构化 Outcome decision 把冻结 Review/source fence、Continue/Stop/Scale/Test、actor/rationale/idempotency、私有 Conversation message、Checkpoint、下一 Task、Event/Outbox 原子绑定。两者都不能被通用 completion API 绕过，也不执行 Provider。VM-08 v4 的 `listing_write_readback` 现使用 typed `effect_readback_v2`：写入阶段只形成 ReceiptCandidate，完成还要求与该候选关联的独立、只读凭据 account readback 和 canonical field diff；ReceiptCandidate、corroboration 或 generic completion 单独都不能完成，缺失或不一致时失败关闭。该合同仍是 E1，且明确不声称 adapter 已注册、Provider 已执行/出具 Receipt 或产品级业务验证。Application Handler Registry 只允许 Catalog 与当前二进制同时注册的 handler 执行；当前 Registry v12 注册 VM-00 `vm00.local-project-identity/v1`、`vm00.local-project-inventory/v1`、`vm00.local-encryption-workspace-ready/v1` 与 `vm00.local-connection-readiness/v1`。前两条完成本地 `identity.resolve` 与精确持久化 workspace inventory，并以 Project revision fence 原子推进；第三条在 Desktop 已证明 exact Device 能打开加密 Context 后，按观察时刻验证 ProjectKeyring 的 mode-specific active recipient set，并以 ProjectKeyring revision fence 原子推进；第四条只重读与已验证 checkout 精确绑定且当前 live 的持久 Connection，以 Connection revision fence 原子完成 `connection_probe` 并进入 Human `goal_selected`。四条 Evidence/Event/Outbox 都只保留 digest、revision 与有界状态，第四条不会调用或模拟 Provider；保留的 VM-11 八条 handler 除 event ingest、normalization、identity chain、typed KPI、attribution、settlement reconciliation 和 deterministic outcome review 外，还注册 `vm11.next-contract-or-valid-terminal/v1`。第七条把父合同、KPI、归因、结算、已验证 Effect 支出、pending Effect 计数和 unresolved cost exposure 冻结为分币种 Review，显式保留 target gap、Unattributed、outstanding settlement、pending Effect、预算超限和无 FX caveat，固定 `causalStatus=not_claimed`、`roiStatus=not_calculated`，且只能 handoff 到 Human `continue_stop_scale_test`，不能替用户选动作。第八条精确绑定 action、decision digest、父 Mission revision/contract digest 与 route revision：Stop 原子形成 typed `Completed` 并把 `candidate_learning` 标为 `Skipped`；Continue 只复用仍为当前的冻结父合同并进入既有合法下一步；Scale/Test 不合成新合同，而把当前 route 保持为 `WaitingUser`，等待另行批准完整 revised/experiment contract。exact replay 不追加 Event/Outbox，来源漂移失败关闭。该第八条现有 Desktop data-plane caller 与窗口动作：窗口把冻结决策 digest、父合同 digest 与 Mission/Checkpoint revision 交给 `resolve_vm11_next_contract_or_valid_terminal`，通用 Application 完成入口继续被拒绝。精确覆盖与缺口由版本化 Registry 和 Docs Machine Truth 计算；未注册的 Application route 显示 `NOT_IMPLEMENTED`，旧 Catalog digest 显示 `BLOCKED_CATALOG_REVISION`。合同到期原子形成 `Schedule Expired + Mission Completed`，非重试故障或第五次租约过期原子形成 `DeadLetter + Mission Partial`。该实现尚不是生产 Scheduler 或 Mission E3：还没有 OS wake/sleep-resume、Cell 多 Worker/leader、公平调度、其余未注册的 Application handler、Effect Broker/Browser handler、其余 Human Checkpoint、redirect 或十二条 Mission 的完整原生 UI Journey。

VM-00 的第四步 `entitlement_and_wallet / billing.checkout` 现已接入 Desktop 的 Cordis 主路径：Application 只接受当前 VM-00 route、精确 Mission/Checkpoint revision、已探测的 provider/account/`billing.checkout` scope、正数且不超过合同预算的 Payment Effect；Desktop 再经 Cordis domain command、显式独立 Approval 和确定性 Effect Broker policy 执行一次 Provider，并在独立 Verification 为 `Confirmed` 后原子完成该 Checkpoint、启动 `connection_probe`。若崩溃发生在 Receipt/Verification 落盘之后，重进只完成 Checkpoint；若已完成则只复用持久结果；Provider `uncertain` 保持 `VerificationRequired`，不会获得第二次 execution。第五步 `connection_probe` 现在由本地 Application handler 重读该 verified checkout 精确绑定的 Connection；缺失、过期或非 live 时可恢复阻塞，Connection 刷新后以同事务 revision fence 完成并进入 Human `goal_selected`，exact replay 不新增 Event，且不调用或伪造 Provider probe。当前测试使用注入式 Stripe 合同身份和 fixture executor/verifier，不包含真实 Stripe 凭据、真实扣款、Wallet/Entitlement 业务记录或 E4 证据；Mission 等级保持 E1。本文其余“Effect Broker/Browser handler 尚缺”均应理解为除这一 VM-00 窄路径外的其余 Effect Broker route 与全部 Browser route。

VM-00 `goal_selected` 现已从通用字符串确认切换为第三条结构化 Human 边界：Desktop 只能提交同一 Project、无 parent 的 VM-01～VM-10 Catalog 合同；Application 从当前 Catalog 编译并冻结目标 Operating Contract、Checkpoint DAG、稳定 Mission/首 Task ID，再把完整计划作为 SQLCipher 私有 Conversation message 与 Human Checkpoint 双 CAS 原子提交。公共 Event/Outbox 只保留 digest；普通文本、VM-00/VM-11 自指、Catalog 漂移、stale revision 与同幂等键换目标均失败关闭。exact replay 不新增 Event，也不创建目标 Mission、启动 Runtime 或调用 Provider；目标的原子创建与最终交接仍是下一切片。此段取代上文“Human route 只有两个边界”和“`goal_selected` 复用通用 Human confirmation”的旧描述。
...
schema v47 完整继承 v35 及此前的 US/EU Cell 注册 Saga、Outcome 来源证明、Context Workspace/Branch/Worker Lease/Capsule、删除/传播、设备附加与 claim-first key bootstrap、Identity 决策历史、Effect rate-limit/reconciliation、Context Foundation/Collaboration、Runtime Recovery/Turn、content-free Assembly、tokenizer hash-only projection 和 Browser File Grant；并增加 v36 MissionDefinition DAG、v37 MissionConversation、v38 Runtime private-message ledger、v39 Runtime Process Claim/cleanup ledger、v40 Mission Schedule ledger、v41 expiry terminal migration、v42 Checkpoint Capability/executor、v43 Checkpoint Oracle/completion policy、v44 Human confirmation message constraint、v45 Outcome Review/Human decision ledger、v46 Runtime private text delta ledger 与 v47 `mission_checkpoints.route_completion_policy` CHECK rebuild。联合迁移门禁覆盖 fresh、v44/v45 顺序经过 v46→v47、既有 v46→v47 的旧 policy/行保留与 typed `effect_readback_v2` reopen，以及碰撞失败时 table/index/data/ledger 整体回滚和清理后重试；迁移账本只各记录一次顺序在后的 `record_migration(45)`、`record_migration(46)`、`record_migration(47)`。版本化 `SyncDocument` 仍以 AAD、tenant/project/object/kind 认证九类 typed projector；Outcome 的订单、退款、付款、身份与归因链继续要求可复算来源证明，事件历史不可改写，本地分叉与 support 写入仍由 savepoint 原子回滚。
...
repair 前已记录的全量 Rust 回归为 **492 passed、0 failed、4 ignored**；同一历史 Catalog Snapshot v2 digest 为 `0955d8873f065882795a39d26c3d9b178892c4f9b1d6e3ba91d9bbd765959dcc`，当时机器报告 Application handler `7/52 implemented、45 NOT_IMPLEMENTED`。绑定 c71061e 的 Release Evidence 2.2 baseline 同样是历史证据，结果仍为 `passed: false`：十二条 Mission 均为 E1/`not_implemented`，V0/V1/V2、横切、E4 与 E5 执行证据均为 0。DOC-47 没有在 repair 后重跑全量测试或生成新 Catalog digest；当前覆盖只以 Registry 和 Docs Machine Truth 的机器校验结果为准，不得把旧 digest、旧测试数或 Release baseline 继承为当前发布证据。VM-11 本地链现到 `event_ingest → normalize_dedupe_order → identity_chain → mission_specific_kpi → attribution_and_unattributed → refund_commission_payout_recalc → outcome_review → Human continue_stop_scale_test → next_contract_or_valid_terminal`：Stop 形成 typed terminal，Continue 复用 exact frozen parent contract，Scale/Test 进入 `WaitingUser` 等待独立合同授权；第八 handler 现有 Desktop caller/UI wiring，Stop/Continue/Scale/Test 仍分别形成 typed terminal、复用冻结父合同或进入 `WaitingUser`。repair 前记录的四个默认忽略真实环境测试（2 个 OpenInterpreter、2 个 Chrome）逐条通过，但仍只证明相应旧 checkpoint 的本地 E2。credentialed OpenInterpreter、Windows 实机、native Keychain、签名身份、PostgreSQL live、原生视觉/AX 与完整 Mission continuity 的限制不变，缺环境时保持 `BLOCKED_ENV`。完整明细见 [Current Worktree Evidence](./docs/quality/CURRENT-WORKTREE-EVIDENCE.md)。
...
2. Context C0/C1、Context Assembler、Runtime Recovery/Process Claim/Turn、durable Mission Scheduler 与 Browser B0/B1a/B1b/B1c/B2-narrow 本地切片已覆盖 Working Set/Continuation/typed invariant/Compaction/Checkpoint、项目作用域 encrypted CAS/File/Query snapshot resolver、digest-pinned tokenizer、runtime provider/model identity binding、最小授权且预算有界的 Runtime envelope、content-free Assembly Manifest、typed Branch merge、Worker usage、bounded Mailbox、detach/reattach epoch fencing、checkpoint/config/thread/process identity binding、bounded retry、exact turn dispatch/stream/local approval/interrupt、startup full-ledger reconciliation、`uncertain` 零重放、cadence/event signal/lease generation、123 个 Checkpoint→Capability/executor/Oracle/policy 路由、route-aware Desktop selection、Human confirmation 原子 handoff、Application handler allow-list、VM-00 本地 Project identity/inventory/encryption/connection-readiness 四段 fence，以及 VM-11 event-ingest/normalization/identity/KPI/attribution/settlement/outcome-review/next-contract-or-valid-terminal 八条多来源 fence；第八条现有 Desktop UI caller，仍不是十二条 Mission 的完整原生 UI Journey。Browser 目前证明 typed Workspace/Profile/Lease/Action、Fake Host、真实 pipe/AX、script-disabled exact-origin navigation、stable semantic locator、单一 Effect-bound semantic click、受控空白文本输入、File Broker→真实 file-input 本地选择、接管恢复、durable File Grant 与 project-local durable Signed Recipe；下一步把当前 exact Device Context session 从 Manifest preview 扩展到任意 encrypted CAS 正文/file/query 浏览、编辑与采用，并补删除传播/重加密/内容扫描，建立正式支持模型的签名 tokenizer artifact registry 和 Provider model-revision 证明，接入 credentialed OpenInterpreter，再补生产 Scheduler 的 OS wake/sleep-resume、Cell 多 Worker/leader、其余未注册的 Application handler、Effect Broker/Browser handler、其余 Human Checkpoint、其他 route 的完成回写与 redirect，以及 provider switch、跨进程并发/`loom`、外部进程 kill/整机断电恢复和 Windows Job Object 实机，最后补 SQLCipher/PostgreSQL 跨后端并发与恢复等价性。真实 PostgreSQL 环境缺失时继续显示 `BLOCKED_ENV`。
