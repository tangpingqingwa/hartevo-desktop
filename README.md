...
3. [Desktop 交互规格](./docs/product/HARTEVO-DESKTOP-INTERACTION-SPEC.md)：当前冻结的产品层级、信息架构和完整交互。
4. [Desktop 当前架构](./docs/architecture/HARTEVO-DESKTOP-ARCHITECTURE.md)：组件所有权、数据流、本地与云边界、安全不变量。
5. [Agent UI 组件采用规范](./docs/design/AI-AGENT-UI-COMPONENT-GUIDE.md)：如何在 Dioxus 中参考 AI CSS，并处理授权、状态语义和可访问性。
6. [质量与 Eval 入口](./docs/quality/README.md)：怎样证明 Mission 真正完成，而不只是界面或工具存在。
7. [可交互原型](./prototype/index.html)：当前产品行为的最终视觉和交互参考。
...
3. MVP Implementation Backlog、Dioxus Shell 与首批 Mission Harness。
