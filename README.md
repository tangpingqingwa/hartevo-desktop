...
repair 前已记录的全量 Rust 回归为 **492 passed、0 failed、4 ignored**；同一历史 Catalog Snapshot v2 digest 为 `0955d8873f065882795a39d26c3d9b178892c4f9b1d6e3ba91d9bbd765959dcc`，当时机器报告 Application handler `7/52 implemented、45 NOT_IMPLEMENTED`。绑定 c71061e 的 Release Evidence 2.2 baseline 同样是历史证据，结果仍为 `passed: false`：十二条 Mission 均为 E1/`not_implemented`，V0/V1/V2、横切、E4 与 E5 执行证据均为 0。DOC-47 没有在 repair 后重跑全量测试或生成新 Catalog digest；当前机器合同只确认 Registry v8 的 8/52 与 44 条 `NOT_IMPLEMENTED`，不得把旧 digest、旧测试数或 Release baseline 继承为当前发布证据。VM-11 本地链现到 `event_ingest → normalize_dedupe_order → identity_chain → mission_specific_kpi → attribution_and_unattributed → refund_commission_payout_recalc → outcome_review → Human continue_stop_scale_test → next_contract_or_valid_terminal`：Stop 形成 typed terminal，Continue 复用 exact frozen parent contract，Scale/Test 进入 `WaitingUser` 等待独立合同授权；第八 handler 已有 Desktop data-plane caller、Mission composer 窗口动作与 data-plane Journey。repair 前记录的四个默认忽略真实环境测试（2 个 OpenInterpreter、2 个 Chrome）逐条通过，但仍只证明相应旧 checkpoint 的本地 E2。credentialed OpenInterpreter、Windows 实机、native Keychain、签名身份、PostgreSQL live、原生视觉/AX 与完整 Mission continuity 的限制不变，缺环境时保持 `BLOCKED_ENV`。完整明细见 [Current Worktree Evidence](./docs/quality/CURRENT-WORKTREE-EVIDENCE.md)。
