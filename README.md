...
Desktop 现在从机器 Catalog 精确启动 VM-00～VM-11，并把 Operating Contract、Checkpoint、Conversation、Context、Runtime Process Claim/Recovery/Turn、Work Product 与 Outcome 保持在同一 Domain。Runtime 的私有 `agentMessage` 与 content-free evidence 同事务持久化；完成草稿的 Work Product/Manifest、Assistant Conversation message、Capsule/Branch/Handle/Lease 终态也同事务提交。用户 correction/steering 会先撤销上一用户代际，覆盖“仅 Context”“Prepared Recovery 无 Turn”“Attached Recovery 无 Turn”和终态 Turn 四类边界；旧代际不会再出现在当前 Runtime 投影中。v39 在 spawn 前提交 Claim，pinned Runtime 使用每 Claim 唯一 launch 副本；启动恢复只有在 PID、start epoch、executable/runtime digest 与私有 token/唯一路径标记同时匹配时才按后代优先有界清理。无法安全检查或终止时保持 `Blocked` 并禁止第二次启动，不会按 PID 猜杀。Desktop 仅投影 Claim 状态与 cleanup 次数，不暴露 PID、令牌或路径。

v40～v44 还实现了本地 durable Mission Scheduler 与精确 Checkpoint 路由的 E2 切片：连续型 Outcome 与下一周期 Schedule 同一事务；interval、event-driven 与 interval-or-event cadence 被强类型化；Conversation signed inbound 与首次事件 signal 同一事务；claim/heartbeat/failure 只接受 exact owner/token digest、generation 与 lease；Catalog 周期只按 n→n+1 重置已终结 DAG。Catalog v10 的 123 个 Checkpoint 均按 DAG 顺序绑定 Capability、executor、精确 Oracle 子集与 completion policy，Application 创建/推进的 Task 必须匹配当前路由；Task/Checkpoint/Event/Outbox 原子提交，旧的未绑定定义只可审计读取，Scheduler 会 `DeadLetter + Partial` 而不会猜 Capability。本地 Runtime 只接受 executor=`runtime`。`dispatch_current_mission_checkpoint` 会把 DAG 中下一个 `Ready` 节点连同 exact Task 原子启动，并返回 revision-fenced proof；Desktop 只有在 Runtime+Ready 时进入 OpenInterpreter。Human route 现在有窄而真实的 `ConfirmHumanMissionCheckpoint`：VM-07 首个 Checkpoint 的用户陈述、双 revision CAS、可选 WorkProduct、完成和下一 route Task 在同一事务，通用 completion API 不能绕过。Application Handler Registry 则只允许 Catalog 与当前二进制同时注册的 handler 执行；当前 `vm11.event_ingest/v2`、`vm11.normalize-dedupe-order/v1`、`vm11.identity-chain/v1`、`vm11.mission-specific-kpi/v1` 与 `vm11.attribution-and-unattributed/v1` 原子物化 Oracle proof。前两条绑定完整 Ledger 并独立复算来源验证、事件去重、规范顺序和订单/退款投影；第三条要求 Outcome 的 Connection、已确认 IdentityLink、Person/Company/Partner、Opportunity/Buying Committee 形成精确传递闭包，退款/佣金只能继承已验证原订单身份，并把所有被读取记录的 revision 与 Mission CAS 同事务 fence；第四条要求 VM-11 显式绑定同项目非 VM-11 父 Mission并完整继承其合同，只按父 Mission 合同窗口、当前身份闭包和验证时点复算 typed count/minor-unit KPI；第五条优先采用经独立 Verification 绑定到精确 Effect 的 link/coupon/provider identity，否则只选合同窗口内最新 source-verified non-direct touchpoint，同时保留 first-touch 和 Unattributed，固定 `causalClaim=false`。父 Mission、触点 Mission、Outcome、Identity 与所有 Effect support revision 都在 Mission CAS 事务中 fence。迁移可读但缺失当前来源验证的旧事件、未确认身份、重复触点、争议身份或伪造 Effect 只会阻塞。其他 47 条 Application route 显示 `NOT_IMPLEMENTED`，旧 Catalog digest 显示 `BLOCKED_CATALOG_REVISION`。合同到期原子形成 `Schedule Expired + Mission Completed`，非重试故障或第五次租约过期原子形成 `DeadLetter + Mission Partial`。该实现尚不是生产 Scheduler 或 Mission E3：还没有 OS wake/sleep-resume、Cell 多 Worker/leader、公平调度、其余 47 条 Application handler、Effect Broker/Browser handler、其余 Human Checkpoint、redirect 或十二条 Mission 的完整原生 UI Journey。
...
当前 worktree 的本地门禁为 `cargo test --workspace --all-targets --all-features --locked` **473 passed、0 failed、4 ignored**；四个默认忽略的真实环境测试（2 个 OpenInterpreter、2 个 Chrome）必须单独记录，不能由默认套件冒充。严格 Clippy、OpenInterpreter source/license/notice/schema/checksum、Catalog/Asset/VS-01 replay 与 Dioxus `.app` bundle 通过；Catalog Snapshot v2 digest 为 `53e656a292011fb279b3df34e8a15bcd956944b2e4d224822384df9c631fa7d4`，自动 Release Evidence 2.2 baseline 仍必须是 `passed: false`，十二条 Mission 仍为 `not_implemented`，并诚实报告 Application handler `5/52 implemented、47 NOT_IMPLEMENTED`。新增测试覆盖 123 个 exact Checkpoint route、Human 双-CAS，以及 VM-11 `event_ingest → normalize_dedupe_order → identity_chain → mission_specific_kpi → attribution_and_unattributed` 的结构化多来源 Oracle proof、严格来源验证、规范化/去重/顺序、身份闭包、typed KPI、verified-identity/non-direct/first-touch/Unattributed 分离、Outcome Ledger/Mission/逐记录 revision fence、防篡改、争议拒绝、冲突恢复、幂等与 Desktop 零 Runtime/Effect；这些仍只证明本地 E2。真实 OpenInterpreter 证据覆盖隔离、无凭据失败关闭、Application 不伪造完成，以及 macOS 本地 pinned Runtime 的 coordinator-handle 丢失/提交间隙恢复；Runtime Adapter 的 Windows x64/ARM64 条件编译检查通过，但全工作区 Windows 交叉构建在 macOS 上被 MSVC C SDK 与 Windows Perl/OpenSSL 工具链阻断，不能算 Windows 产品证据。当前证据仍不覆盖 credentialed success、生产模型、provider switch、外部 `SIGKILL`/整机断电、Windows 实机或完整 Mission continuity。显式 native Keychain smoke 仍返回 `BackendUnavailable`，本机 codesigning identity 为 0；原生窗口视觉/AX 仍不能由 bundle 代替。Windows Credential Manager 与跨平台安装矩阵未实测。PostgreSQL live gate 因缺 `HARTEVO_TEST_POSTGRES_URL` 同样保持 `BLOCKED_ENV`。完整明细见 [Current Worktree Evidence](./docs/quality/CURRENT-WORKTREE-EVIDENCE.md)。
