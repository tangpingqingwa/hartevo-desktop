...
尺寸、时长或版本检查失败的素材保留供检查，但不能采用。视频生成中可关闭工作台或退出应用；重开后“继续取回视频”查询同一任务，不重新生成。结果不明确的同步请求显示明确状态，系统不自动重发。工作台重开和刷新同步最新 Mission 与素材状态；重复查看同一已加载素材不会取消采用资格。生成期间项目被锁定时只保存原回执或隔离字节，不继续展示或采用。
