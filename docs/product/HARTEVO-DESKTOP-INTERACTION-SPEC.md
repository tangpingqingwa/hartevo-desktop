...
### 任务与成果的默认呈现（2026-09-07 体验整改）

默认任务页依次展示目标、事实状态、当前成果与会话。Agent Operations、运行配置、处理轨迹与 manifest 等技术信息移入“运行详情”。工具、本地写入、外部操作和浏览器需要用户处理时必须在主页面保留明确入口。普通任务描述使用中文产品语言，不能要求用户理解 Runtime、Capability 或 SQLCipher。

“正在处理”需要当前任务有活动执行；已有素材可显示“待审阅”或“成果已保存”，不将 Mission 的 Running 阶段视为实时执行证明。任务列表支持全部、进行中、待确认、已结束与搜索；待确认包含待用户处理的状态、待批准操作及待审阅成果。

成果卡使用本地解锁上下文的真实预览。打开卡片后，工作台定位同一 WorkProduct 的当前生成 ID。工作台先展示当前图片/视频，将历史版本及失败记录折叠；“新建素材”“再做一版”“修改描述”打开有焦点的生成表单。导出通过系统保存对话框，在用户选定路径后再次校验上下文、类型、字节长度与摘要，成功才保存文件。

旧 bootstrap 任务没有对话时，不显示不可发送的普通输入框，改为明确的创意工作台入口。有对话但模型未连接时提供直达模型设置入口。统一多模型配置、旧任务对话迁移和 Agent 自动素材调用仍是缺口，详见[体验整改验收](../quality/PRODUCT-EXPERIENCE-REBUILD.md)。

### Mission 创意素材工作台（2026-09-07）

已解锁的运行中 Mission 在 Workpad 提供“创意素材”：选择 GPT 图片、Grok 图片或 Grok 短视频，描述画面后明确点击生成。素材任务保留在当前 Mission；用户可查看历史画面、修改描述生成新版本，在图片加载或视频首帧解码成功后采用当前版本。后续播放错误会撤销采用资格，通用结果区与 Operations 只能打开媒体工作台。修改描述是重新生成，不承诺保持原图像素或执行视频剪辑；已采用版本不被覆盖。

尺寸、时长或版本检查失败的素材保留供检查，但不能采用。视频生成中可关闭工作台或退出应用；重开后“取回视频”查询同一任务，不重新生成。结果不明确的同步请求显示明确状态，系统不自动重发。工作台重开和刷新同步最新 Mission 与素材状态；重复查看同一已加载素材不会取消采用资格。生成期间项目被锁定时只保存原回执或隔离字节，不继续展示或采用。

此入口沿用当前 Mission，并仅形成待审阅的创意 WorkProduct；采用不会自动发布、发送消息或完成业务目标。模型连接由开发环境配置，尚无新增连接设置界面或可自动调用的媒体 Agent 工具。
