...
| E22 | Connections overview | need-next rows + verified/problem rows + right rail | connect/re-auth/detail | 3/4 stats, compact rows | Connection projections | `PARTIAL(P1)`；need-next/统计/右 rail 密度完成；无真实 Probe 时始终 0 Connected/`BLOCKED_ENV` | only live Probe may say Connected |
| E23 | Add connection modal | four-step wizard | purpose→account→auth→probe | 620px modal, stepper | Application auth state | `PARTIAL(P1)`；四步 modal、返回/继续/关闭与授权边界完成，真实 OAuth/account/probe 未接线 | cancel/expired/wrong-account/revoke states |
| E24 | Connections subviews | all/policy/activity | filter/open details | compact table/list | Connection/Policy/Audit projections | `PARTIAL(P1)`；三子视图与 policy matrix 完成，真实 audit/secret projection 未接线 | secrets absent from DOM/AX/log |
