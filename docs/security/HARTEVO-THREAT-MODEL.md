...
| Worker/Branch/Runtime/Scheduler 过期写入、executor 混淆或伪造 Checkpoint 完成 | Worker/Branch/Capsule 继续以 token digest、generation、revision、lineage、attachment epoch、strict cursor、预算与有界 Mailbox 失败关闭；Runtime 继续以清洗环境、correlation、protocol poison、Process Claim、exact process identity/token marker 和 bounded retry 阻断旧代际与 PID 猜杀。schema v40～v44 把 Schedule、Capability/executor、Oracle/policy 与 Human confirmation 分层持久化；Catalog v10 的 123 个 route 必须 Capability/Oracle 并集闭合，Runtime/Effect route 分别要求真实 WorkProduct/verified Effect，legacy route 可审计但不能完成。Human route 只能通过 Mission+Checkpoint+Conversation revision 与 route digest 绑定的原子命令。Application route 还要求 Registry+二进制双注册和当前 Catalog digest；VM-11 `event_ingest`、`normalize_dedupe_order`、`identity_chain`、`mission_specific_kpi` 与 `attribution_and_unattributed` 的 Mission/Checkpoint/Outcome/Identity/父 Mission/触点 Mission/Effect support proof、来源 fence、Checkpoint、next Task、Event/Outbox 同事务，actual sourceKinds 与逐来源 Oracle 责任必须等于 Registry。第三条 handler 要求 Connection、Confirmed IdentityLink、Person/Company/Partner、Opportunity/Buying Committee 是精确闭包，外部 provider/account 一致，退款/佣金只继承已验证原订单身份；第四条要求显式父 Mission、完全继承合同，并按合同窗口、验证 cutoff、当前身份闭包和 `minor_units:ISO` 复算 KPI；第五条只允许 source-verified touchpoint，VerifiedIdentity 必须绑定已批准、已 Receipt、独立确认的精确 Effect/provider/payload，否则只使用 last-non-direct，并始终保留 first-touch/Unattributed、禁止因果宣称；每个来源 revision 逐条 fence。迁移可读的未验证来源、未确认链接、错账号、父合同漂移、未知 KPI、币种冲突、重复/争议归因、伪造 Effect、scope/digest tamper 和 stale source 都不能升级为完成，其他 47 条明确 `NOT_IMPLEMENTED`。旧 lease 不能追加 failure 或启动新 cycle，`uncertain` 不重放外部 Effect。仍缺 credentialed Runtime、生产 OS/Cell Scheduler、其余 47 条 Application handler、Effect Broker/Browser handler、其余 Human route、外部 `SIGKILL`/整机断电、Windows Job Object 与 PostgreSQL 等价性 |
