...
- Desktop 当前使用 SQLCipher schema v46，并继承 Cell 注册/密文同步、删除、密钥、Effect、Context、Browser、Runtime 与 Mission Schedule 的既有 DR 合同。v42 保存 Checkpoint Capability/executor；v43 保存 Oracle/completion policy；v44 保存受约束的 Human confirmation message；v45 保存 VM-11 Outcome Review/source fence/structured decision；v46 保存 Runtime private text delta/evidence/private-message chain。v42 legacy route 迁移后可审计但不可完成；v43～v46 均保留旧行并验证幂等 reopen；联合门禁覆盖 fresh、v44→v45→v46、既有 v45→v46、加密备份与失败回滚。
...
- Catalog v10 route 恢复必须重验 exact Capability/executor/Oracle/policy。Human route 只能通过双 revision CAS 命令，把 Conversation、Checkpoint、旧/新 Task 与 Event/Outbox 原子提交；通用完成入口不能绕过。Application handler 还必须同时存在于版本化 Registry 与当前二进制，并匹配 Mission Catalog digest；当前 VM-11 `event_ingest`、`normalize_dedupe_order`、`identity_chain`、`mission_specific_kpi`、`attribution_and_unattributed`、`refund_commission_payout_recalc` 和 `outcome_review` 都以来源 revision fence 与 Mission CAS 同事务，实际 Oracle sourceKinds 还必须与 Registry 完全相等。第三条逐条 fence Connection/IdentityLink/Person/Company/Partner/Opportunity revision；第四条另外 fence 显式父 Mission，并重算其合同窗口、当前身份闭包与 typed count/minor-unit KPI；第五条 fence 父/触点 Mission、Outcome、Identity 与 Effect support，恢复时重新验证 Receipt/readback/payload identity，区分 VerifiedIdentity、last-non-direct、first-touch 和 Unattributed，且固定禁止因果宣称；第六条重验不可变订单、跨期退款、current refund-set Commission revision、Partner Supply Class authority 与 source-verified payout 的 partner/currency/provider 对账，并且不把对账投影冒充付款审批；第七条从相同父 Mission、Outcome ledger、Identity closure 与 observation cutoff 重新计算 KPI、Attribution、Settlement、Effect cost 和预算，各币种独立保存，拒绝隐式 FX、ROI、因果或自动决策。随后 Human decision 以冻结 Review/source、双 CAS、Continue/Stop/Scale/Test、actor/rationale/idempotency 原子写入，但 `next_contract_or_valid_terminal` 仍未实现。Missing source 的“应不存在”状态同样进入事务 fence，避免恢复竞态固化过期阻塞。其他 45 条为 `NOT_IMPLEMENTED`。故障注入覆盖 route、Human message、Application source、Task 与 Event 整体回滚；缺当前来源验证的迁移 Outcome 只能进入 `outcome_source_unverified` 阻塞，未确认身份进入 `identity_link_unconfirmed`；父合同漂移、未知 KPI、币种冲突、无父 Mission Outcome、重复/争议归因、错误 Effect support、stale commission、重复/超额 payout、settlement authority 混用或 Review 来源/币种/预算不一致都只能 durable block。未覆盖 credentialed Runtime、active-script/authenticated Provider、Windows pipe/reparse/Job Object、生产 scanner、Provider readback/Verification、外部 process-kill/整机断电、真实 PostgreSQL DR、OS/Cell Scheduler、其余 45 条 Application handler、Effect Broker/Browser handler、其余 Human route、真实 Dioxus delta 投影与跨后端等价性；无 `HARTEVO_TEST_POSTGRES_URL` 时保持 `BLOCKED_ENV`。
