...
- Desktop 当前使用 SQLCipher schema v47，并继承 Cell 注册/密文同步、删除、密钥、Effect、Context、Browser、Runtime 与 Mission Schedule 的既有 DR 合同。v42 保存 Checkpoint Capability/executor；v43 保存 Oracle/completion policy；v44 保存受约束的 Human confirmation message；v45 保存 VM-11 Outcome Review/source fence/structured decision；v46 保存 Runtime private text delta/evidence/private-message chain；v47 只事务性重建 `mission_checkpoints` 的 completion-policy CHECK，把 `effect_readback_v2` 加入 allow-list，保留既有 15 列、PK/UNIQUE/FK、状态/完成约束和索引。v42 legacy route 迁移后可审计但不可完成；联合门禁覆盖 fresh、v44/v45→v46→v47、既有 v46 policy/行保留与 typed route reopen。注入 `mission_checkpoints_v47` 碰撞时，table rename、index、数据与 `record_migration(47)` 整体回滚；移除碰撞后可重试，45→46→47 各只有一个顺序 ledger 记录。
...
- Catalog v10 route 恢复必须重验 exact Capability/executor/Oracle/policy。Human route 只能通过双 revision CAS 命令，把 Conversation、Checkpoint、旧/新 Task 与 Event/Outbox 原子提交；通用完成入口不能绕过。VM-08 v4 的 `effect_readback_v2` 仍是 E1 合同：write ReceiptCandidate 必须与独立、只读 credential 的 account readback 和 canonical field diff 关联；ReceiptCandidate、corroboration、已验证 Effect 或 generic completion 单独均不授予完成，缺失/不一致失败关闭，且不声称 Provider/产品验证已经发生。Application handler 还必须同时存在于版本化 Registry 与当前二进制，并匹配 Mission Catalog digest；v14 当前注册 VM-00 `vm00.local-project-identity/v1`、`vm00.local-project-inventory/v1`、`vm00.local-encryption-workspace-ready/v1` 与 `vm00.local-connection-readiness/v1` 与 `vm00.local-operating-contract-compiled/v1` 与 `vm00.local-mission-handoff/v1`，前两条以本地 Project revision fence 依次完成 `identity.resolve` 与 `project_inventory`；第三条在 Desktop exact Device Context gate 之后验证持久化 ProjectKeyring readiness，并以 ProjectKeyring revision fence 完成 `encryption_workspace_ready`；第四条只重读 exact verified-checkout Connection，缺失/过期时可恢复阻塞，刷新后以 Connection revision fence 完成 `connection_probe` 并进入 Human `goal_selected`，exact replay 不新增 Event，也不调用 Provider；第五条在既有 Human `goal_selected` 双 CAS 后验证当前 Operating Contract、冻结定义与 confirmation proof，以 Mission revision fence 原子进入 `mission_handoff`，不保存私有目标或确认正文；第六条绑定同一当前合同和紧邻的编译完成证明，以 Mission revision fence 原子完成最终 Checkpoint，保持 `Verifying` 且不创建目标 Mission、不启动 Runtime/Provider；另注册 VM-11 `event_ingest`、`normalize_dedupe_order`、`identity_chain`、`mission_specific_kpi`、`attribution_and_unattributed`、`refund_commission_payout_recalc`、`outcome_review` 和 `next-contract-or-valid-terminal`，实际 Oracle sourceKinds 还必须与 Registry 完全相等。前七条继续以来源 revision fence 与 Mission CAS 同事务；第三条逐条 fence Connection/IdentityLink/Person/Company/Partner/Opportunity revision；第四条另外 fence 显式父 Mission，并重算其合同窗口、当前身份闭包与 typed count/minor-unit KPI；第五条 fence 父/触点 Mission、Outcome、Identity 与 Effect support，恢复时重新验证 Receipt/readback/payload identity，区分 VerifiedIdentity、last-non-direct、first-touch 和 Unattributed，且固定禁止因果宣称；第六条重验不可变订单、跨期退款、current refund-set Commission revision、Partner Supply Class authority 与 source-verified payout 的 partner/currency/provider 对账，并且不把对账投影冒充付款审批；第七条从相同父 Mission、Outcome ledger、Identity closure 与 observation cutoff 重新计算 KPI、Attribution、Settlement、Effect cost 和预算，各币种独立保存，拒绝隐式 FX、ROI、因果或自动决策。Human decision 后，第八条精确重验 action、decision digest、父 Mission revision/contract digest 与 route revision：Stop 原子完成并跳过 `candidate_learning`；Continue 复用 exact frozen parent contract 并进入既有下一步；Scale/Test 保持 `WaitingUser`，等待另行批准完整 revised/experiment contract。exact replay 零新增 Event/Outbox，source drift 失败关闭；Desktop 现有该 handler 的 caller/UI wiring，仍不构成十二条 Mission 的完整原生 UI Journey。Missing source 的“应不存在”状态同样进入事务 fence，避免恢复竞态固化过期阻塞。机器合同覆盖 14/52，其他 38 条为 `NOT_IMPLEMENTED`。故障注入覆盖 route、Human message、Application source、Task 与 Event 整体回滚；缺当前来源验证的迁移 Outcome 只能进入 `outcome_source_unverified` 阻塞，未确认身份进入 `identity_link_unconfirmed`；父合同漂移、未知 KPI、币种冲突、无父 Mission Outcome、重复/争议归因、错误 Effect support、stale commission、重复/超额 payout、settlement authority 混用或 Review 来源/币种/预算不一致都只能 durable block。未覆盖 credentialed Runtime、active-script/authenticated Provider、Windows pipe/reparse/Job Object、生产 scanner、真实 Provider readback/Verification、外部 process-kill/整机断电、真实 PostgreSQL DR、OS/Cell Scheduler、其余 38 条 Application handler、Effect Broker/Browser handler、其余 Human route、真实 Dioxus delta 投影与跨后端等价性；无 `HARTEVO_TEST_POSTGRES_URL` 时保持 `BLOCKED_ENV`。
- VM-00 `goal_selected` 恢复不再接受普通 Conversation 文本。只允许 `ConfirmVm00GoalSelection` 携带同一 Project、无 parent 的 VM-01～VM-10 typed contract facts和 exact 三重 revision；Application 必须匹配当前 VM-00 v3/Catalog digest，并把冻结 target ID、首 Task ID、resolved title、Operating Contract 与 MissionDefinition 作为 `hartevo-vm00-target-mission-plan/v1` 保存在 SQLCipher 私有 message。恢复使用原始 plan，不按新时间或新 Catalog 重编译；相同幂等键换目标、stale revision、Catalog drift 或私有 plan 不匹配均失败关闭。当前只恢复到可重放计划，禁止据此声称目标 Mission 已创建、VM-00 已终结或 Runtime/Provider 已启动。
