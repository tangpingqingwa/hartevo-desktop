...
- Desktop 当前使用 SQLCipher schema v48，并继承 Cell 注册/密文同步、删除、密钥、Effect、Context、Browser、Runtime 与 Mission Schedule 的既有 DR 合同。v42 保存 Checkpoint Capability/executor；v43 保存 Oracle/completion policy；v44 保存受约束的 Human confirmation message；v45 保存 VM-11 Outcome Review/source fence/structured decision；v46 保存 Runtime private text delta/evidence/private-message chain；v47 事务性重建 `mission_checkpoints` 的 completion-policy CHECK，把 `effect_readback_v2` 加入 allow-list；v48 保存 Keycloak-first identity account/team/membership/project/device/session projections、OS SecretReference binding、offline/expired/revoked status 与 exact revision fences，token bytes 仍只在 OS Secret Store。v42 legacy route 迁移后可审计但不可完成；联合门禁覆盖 fresh、v44/v45→v46→v47→v48、既有 v46 policy/行保留、typed route reopen 与 file-backed identity bootstrap/restart/offline/revoke journey。注入 `mission_checkpoints_v47` 碰撞时，table rename、index、数据与 `record_migration(47)` 整体回滚；移除碰撞后可重试，45→46→47→48 各只有一个顺序 ledger 记录。
